...
import { formatToApiDate } from "../../pages/utils/dateHelper";
...
import FilterActions from "../DashboardSales/FilterActions";
import { clearSalesData } from "../../slices/dashboardSales/reducer";

const DashboardSalesBranch = () => {
  ...
  const yesterdayRevenue = sales
    .filter(
      (item) => getDateOnly(item.transaction_Date) === yesterday.toDateString(),
    )
    .reduce((sum, item) => sum + (item.revenue || 0), 0);
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb
            ...
            subtitle={
              <>
                {sales.find((s) => s.branch_ID === Number(branchId))
                  ?.brancch_Name || branchId}
              </>
            }
          />
          <Row>
...
export default DashboardSalesBranch;
