...
const SalesForecastCharts = ({ dataColors, series }) => {
    let areachartSalesColors = [];
    try {
        areachartSalesColors = dataColors ? getChartColorsArray(dataColors) : [];
    } catch (error) {
        console.warn('Chart colors parsing failed:', error);
        areachartSalesColors = ['#f06548', '#40bb82'];
    }
...
const DealTypeCharts = ({ dataColors, series }) => {
    let dealTypeChartsColors = [];
    try {
        dealTypeChartsColors = dataColors ? getChartColorsArray(dataColors) : [];
    } catch (error) {
        console.warn('Chart colors parsing failed:', error);
        dealTypeChartsColors = ['#f06548', '#40bb82', '#246bfd'];
    }
...
const BalanceOverviewCharts = ({ dataColors, series }) => {
    let revenueExpensesChartsColors = [];
    try {
        revenueExpensesChartsColors = dataColors ? getChartColorsArray(dataColors) : [];
    } catch (error) {
        console.warn('Chart colors parsing failed:', error);
        revenueExpensesChartsColors = ['#f06548', '#40bb82'];
    }
